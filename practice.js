...
var evenOdd = (...parameters) => {
	var even = [];
	var odd = [];
	for (var i = 0; i < parameters.length; i++) {
		if (parameters[i] % 2 === 0) {
			even.push(parameters[i]);
		} else {
			odd.push(parameters[i]);
		}
	};
	return {even, odd};
};
...
var multiply = (num, ...nums) => {
	var returnArr = [];
	for (var i = 0; i < nums.length; i++) {
		returnArr.push(num*nums[i]);
	}
	return returnArr;
};
